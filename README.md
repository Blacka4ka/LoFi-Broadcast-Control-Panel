<img width="1488" height="1241" alt="image" src="https://github.com/user-attachments/assets/d4fe5ad7-1818-48bf-b7c2-85fd04759938" />
...
- періодичний показ анімацій без перемикання музики;
- розклад денних і нічних відеофонів із перемиканням після поточного треку;
...
```bash
sudo apt install -y ffmpeg nginx python3 python3-venv python3-pip rsync ufw snapd fonts-dejavu-core
```
...
Для анімації на кшталт «Підпишись» можна вибрати постійний показ або
автоматичний показ на задану кількість секунд кожні 30, 60, 90 чи
120 хвилин. Після початкового застосування розклад працює всередині
FFmpeg і не перемикає поточний музичний трек.

Коли за розкладом настає час змінити денний або нічний відеофон, worker
чекає завершення поточного музичного треку. Новий фон запускається разом
із наступним треком, тому пісня не обривається посередині. У статусі
панелі під час очікування показується приблизний час до перемикання.

Погодні оверлеї використовують локальні DejaVu-шрифти з підтримкою
української мови та символів погоди. Формат віджета:

```text
Яремче  ·  10°C  ·  ☂  ·  ↗ 6 км/год
```

У панелі можна вибрати сучасний, жирний, класичний або моноширинний
варіант шрифту. Перша іконка показує стан погоди, а стрілка — напрямок
вітру.

### Оптимізація відеофонів

Для стріму 1080p30 найкраще заздалегідь готувати фони у форматі H.264,
`1920x1080`, `30 FPS`, `yuv420p`, без звукової доріжки. Файл у 60 FPS
змушує сервер декодувати вдвічі більше кадрів, хоча вихідний стрім усе
одно працює у 30 FPS.
...
ffprobe -v error -select_streams v:0 \
  ...
  /home/lofi/app/video/ВАШ_ФОН.mp4
```

Оптимізувати важкий або 60 FPS фон:

```bash
ffmpeg -i /home/lofi/app/video/ВАШ_ФОН.mp4 \
  -an -vf "fps=30,scale=1920:1080:flags=lanczos,format=yuv420p" \
  -c:v libx264 -preset medium -crf 21 \
  -g 60 -keyint_min 60 -sc_threshold 0 \
  -movflags +faststart \
  /home/lofi/app/video/ВАШ_ФОН-30fps.mp4
```

Після перевірки нового файла додайте його через сканування папок у
панелі. Не перекодовуйте відео під час активного стріму: ця операція
сама використовує багато CPU.

Для прозорих WebM-оверлеїв рекомендовано:

- VP8 або VP9 з `alpha_mode=1`;
- 15 FPS;
- ширина 300–450 px;
- тривалість 8–12 секунд;
- показ раз на 60–120 хвилин замість постійного, якщо логотип не
  повинен бути на екрані весь час.

Поточний worker обмежує декодування оверлея реальною швидкістю,
обробляє його у 15 FPS і кешує інформацію про кодек. Це не дозволяє
короткій зацикленій анімації забирати всі ядра процесора.
...
Перезапускайте лише сервіс, код якого змінився. Зайвий перезапуск
`lofi-worker` перериває RTMP-сесію та поточний музичний трек.

Після зміни вебпанелі:

```bash
sudo chown -R lofi:lofi /home/lofi/app
sudo systemctl restart lofi-web
```

Після зміни `worker.py`:
...
Після зміни Telegram-бота:

```bash
sudo systemctl restart lofi-bot
```
